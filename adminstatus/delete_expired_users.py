...
for r in res:
	...
	try:
		...
		continue
	if (datetime.datetime.now() >= user_expire):
		print("DELETE: {} --> {}".format(user_name, user_expire))
		try:
			dn = "uid={},dc=example,dc=org".format(user_name)
			con.delete_s(dn)
		except:
			print("ERROR: Unable to delete user: {}".format(user_name))
			exit_status = 1

if (exit_status != 0):
	print("There were problems deleting users")
